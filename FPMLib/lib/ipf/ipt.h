...
public:
	...
	{
		for(int ci=0;ci<cn;++ci)
			//pox[ci]=_CastT::cast<_OPValT, _IPValT>(pix[ci]*m_scale);
			pox[ci] = static_cast<_OPValT>(pix[ci]*m_scale);
	}
};
...
public:
	...
	{
		for(int ci=0;ci<m_cn;++ci)
			pox[ci]=_CastT::cast	(pix[ci]*m_scale);
	}
};
//
							...
							)
{
	...
	{
		for(int yi=0;yi<height;++yi,pSrc0+=istride0,pSrc1+=istride1,pDest+=ostride,pMask+=mstride)
		{
			const _SrcValT0 *pix0=pSrc0;
			const _SrcValT1 *pix1=pSrc1;
			_DestValT *pdx=pDest;
