...
class Vector
	:public Array<_ValT,size>
{
	typedef Array<_ValT,size>  _MyBaseT;
	typedef Vector<_ValT,size> _MyT;
protected:
...
public:
	...
	explicit Vector(const Vector<_Tx,size>& right)
		:_MyBaseT(right)
	{
	}
private:
...
public:
	...
	_CParamT Elem() const
	{
		return this->_ElemX(idx,FlagType<(idx<size)>());
	}
	_CParamT X() const
	{
		return this->Elem<0>();
